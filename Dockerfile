...
# Python packages
RUN conda install --yes numpy pandas scikit-learn scikit-image matplotlib scipy seaborn sympy cython patsy statsmodels cloudpickle dill numba bokeh rasterio shapely fiona && conda clean -yt

# More Python packages
RUN pip install --user obspy geopandas
RUN pip install --user basemap --allow-external basemap --allow-unverified basemap
RUN pip install --user pyke --allow-external pyke --allow-unverified pyke
RUN pip install --user PIL --allow-external PIL --allow-unverified PIL
